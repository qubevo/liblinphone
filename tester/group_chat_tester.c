...
static void chat_room_state_changed (LinphoneChatRoom *cr, LinphoneChatRoomState newState) {
	...
	switch (newState) {
		...
		case LinphoneChatRoomStateExhumePending:
			manager->stat.number_of_LinphoneChatRoomStateExhumePending++;
			break;
...
static void exhume_one_to_one_chat_room_1(void) {
	...
	if (marieOneToOneCr) {
		linphone_core_manager_delete_chat_room(marie, marieOneToOneCr, coresList);
		wait_for_until(marie->lc, NULL, &marie->stat.number_of_LinphoneChatRoomStateTerminated, 1, 5000);
		...
		BC_ASSERT_TRUE(wait_for_list(coresList, &pauline->stat.number_of_LinphoneChatRoomStateExhumePending, 1, 5000));
		BC_ASSERT_TRUE(wait_for_list(coresList, &pauline->stat.number_of_LinphoneChatRoomStateCreated, 2, 5000));

		BC_ASSERT_TRUE(wait_for_list(coresList, &marie->stat.number_of_LinphoneChatRoomStateCreated, 2, 5000));
		BC_ASSERT_TRUE(wait_for_list(coresList, &marie->stat.number_of_LinphoneIsComposingActiveReceived, 1, 5000));
...
static void exhume_one_to_one_chat_room_2(void) {
	...
	if (marieOneToOneCr) {
		linphone_core_manager_delete_chat_room(marie, marieOneToOneCr, coresList);
		wait_for_until(marie->lc, NULL, &marie->stat.number_of_LinphoneChatRoomStateTerminated, 1, 5000);
		wait_for_until(marie->lc, NULL, &marie->stat.number_of_LinphoneChatRoomStateDeleted, 1, 5000);
		BC_ASSERT_EQUAL(linphone_chat_room_get_state(paulineOneToOneCr), LinphoneChatRoomStateTerminated, int, "%d");
